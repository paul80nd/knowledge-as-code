...
* **The table and the frontmatter say the same thing, and CI checks it.** `related-matches-section` reconciles
  ...
  write it in both.
* **Work items live in the table alone.** There is no field for them. Write each issue as an inline link labelled
  `gh#2101`, so the URL travels with the record into an export.
