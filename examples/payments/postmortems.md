...
Publishing one closes it, as with an ADR: a postmortem holds what we understood at the time. Where we later
...
One incident routinely produces a [fix](fixes.md), a runbook, a revised [NFR](nfrs.md) and sometimes an
ADR. A root cause that recurs shows up in no single account, so read several postmortems together when you
...
* **Runbook**: instructions for an incident that might happen. A postmortem is an account of one that
  did.
