...
independently deployed apps gets three documents. An offering spanning six services gets an offering document that
links to all six.
...
| `component-type` *  | `api`                                    | The sort of component this is, which `platform` does not say.                                        |
| `repos` *           | list                                     | Repositories a change to this service is made in.                                                    |
| `platform` *        | `dotnet`                                 | What it is built on. Drives which standards apply.                                                   |
...
Every deployable here is built on .NET, so the list holds `dotnet` alone.
...
Every deployable here is an API, so the list holds `api` alone.
