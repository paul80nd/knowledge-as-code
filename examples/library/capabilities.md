...
* **Explanation**: how something works internally. A capability is what it does externally.
...
* **The table and the frontmatter say the same thing, and CI checks it.** `related-matches-section` reconciles
  ...
  write it in both.
* **Work items live in the table alone.** There is no field for them. Write each epic as an inline link labelled
  `ADO#1150`, so the URL travels with the record into an export.
