...
* **A rule about using the term.** That is a standard.
* **A full explanation of a pattern.** That is an explanation. A glossary entry is a sentence, and links out for the
  rest.
...
8. Name the owning [service](services.md) where the concept has one.
...
| `status` *†   | `draft` `active` | `draft` while the terms are still settling. In force at `active`.             |
