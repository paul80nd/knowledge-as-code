...
We classify data by how sensitive it is, and handle each class accordingly. Personal data adds the obligations we owe
to the people it describes.

Handling every record the same way over-protects trivial data and under-protects the records whose loss would cause
harm. Until we say which data is which, we cannot protect it in proportion to its risk.

## Scope

All data kept or processed by systems we build or operate, in every environment, including backups, exports, logs,
analytical copies and test data.

_Boundary: [pol-ENVS] governs the separation between the environments this data moves through, and [pol-SCRT] governs
the secrets that protect it. [pol-INCR] owns recording a personal data breach and notifying it. This policy owns what
the data itself requires, in whichever environment it is kept._
...
| `RIGHTS`  | **MUST** be able to find, export, correct, delete and restrict one person's data in every store that contains it                             | [UK GDPR].Art.15, [UK GDPR].Art.16, [UK GDPR].Art.17, [UK GDPR].Art.18, [UK GDPR].Art.20, [UK GDPR].Art.21        |
| `INVENT`  | **MUST** maintain an inventory of the information we keep, naming what it is, where it is kept, who owns it and who we share it with         | [ISO 27001:2022].A.5.9, [UK GDPR].Art.30                                                                          |
| `ARCHIVE` | **MUST** retain an information asset's inventory record after the system or store it describes is decommissioned                             | [ISO 27001:2022].A.5.9                                                                                            |
| `XBORDER` | **MUST** keep and process personal data only where a lawful transfer mechanism covers it                                                     | [UK GDPR].Art.44                                                                                                  |
...
| `LOGGED`  | **MUST** log a deletion of classified or sensitive data: who deleted it, when, and who authorised it                                         | [ISO 27001:2022].A.8.10                                                                                           |
| `VERIFY`  | **MUST** obtain verification that a third party has deleted data it held for us, when a contract or request requires the deletion            | [ISO 27001:2022].A.8.10                                                                                           |
| `BULK`    | **MUST** control bulk copying or export of sensitive data, even through a channel authorised for other use                                   | [ISO 27001:2022].A.8.12                                                                                           |
| `SUNSET`  | **MUST** dispose of or sanitise data properly when the system or store holding it is decommissioned                                          | [ISO 27001:2022].A.5.9                                                                                            |
| `UNMASK`  | **MUST NOT** place unmasked production or personal data into an environment below production ([pol-ENVS].UNMASK states the same prohibition) | [ISO 27001:2022].A.8.11, [UK GDPR].Art.25                                                                         |
...
| `LOGS`    | **MUST NOT** write unmasked sensitive personal data into logs or telemetry ([pol-SCRT].LOGS states the same for secrets)                     | [ISO 27001:2022].A.8.12                                                                                           |
| `LEAK`    | SHOULD detect sensitive data leaving through a route that is open for other traffic                                                          | [ISO 27001:2022].A.8.12                                                                                           |
| `TAGGED`  | SHOULD carry a data element's classification as an attribute of the system or store that holds it, not only as documentation                 | [ISO 27001:2022].A.5.12                                                                                           |
...
Where a defect genuinely cannot be reproduced without production data, a time-boxed, access-restricted copy is allowed
