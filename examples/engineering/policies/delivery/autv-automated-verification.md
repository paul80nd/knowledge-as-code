...
| `MAPPED`  | **MUST** be able to trace every stated requirement to the test that validates it                                                        | [ISO 27001:2022].A.8.29                                                                          |
| `REGRESS` | **MUST** add a regression test for every defect we fix, so it can only be found once ([pol-VURM].REGRESS states the vulnerability case) | [ISO 27001:2022].A.8.29, [DORA metrics].change-failure-rate, [NIST SSDF 1.1].PW.8                |
...
| `FLOW`    | SHOULD measure how long a change takes to reach production, and how often a change fails there                                          | [DORA metrics].change-failure-rate, [DORA metrics].lead-time                                     |
| `WARN`    | SHOULD treat a new warning as a defect to triage rather than noise to accumulate                                                        |                                                                                                  |
| `COVER`   | SHOULD measure what proportion of our code the tests exercise, and block a change that lowers it below the figure we set                |                                                                                                  |
| `BITWISE` | COULD produce byte-for-byte identical output from the same source                                                                       | [NIST SSDF 1.1].PW.6                                                                             |
| `FAULTS`  | COULD deliberately break code the tests cover, and check that the tests fail                                                            |                                                                                                  |
...
[pol-VURM]: ../security/vurm-vulnerability-remediation.md#clauses
