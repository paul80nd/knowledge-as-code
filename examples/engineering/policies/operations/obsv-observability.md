...
| `RETAIN`  | **MUST** retain telemetry for a defined period: long enough to investigate, no longer than justified                                                        | [Azure WAF].operational-excellence                          |
| `AUDIT`   | **MUST** retain security-relevant telemetry for at least one year, with the last 90 days immediately available for analysis                                 | [ISO 27001:2022].A.8.15                                     |
...
| `SECRETS` | **MUST NOT** write secrets, credentials or unmasked sensitive personal data into telemetry ([pol-SCRT].LOGS and [pol-DATA].LOGS state the same prohibition) | [ISO 27001:2022].A.8.15                                     |
| `TAMPER`  | **MUST NOT** let an administrator erase or deactivate logs of their own activity                                                                            | [ISO 27001:2022].A.8.15                                     |
