...
replaces a version it already holds.
...
`ContentVersionTests` in `tooling/kac.tests` compares the branch against the commit it grew from, in three steps:

* It asks `git diff` for every file the branch added, changed, renamed or deleted since the merge base, and
  `git ls-files` for a file the working tree keeps that nobody has committed.
* It keeps the files that are records of a type declaring an `export:` block, in a corpus under `examples/`.
* For each corpus left, it reads `content-version` from `.corpus.yaml` at both revisions, and fails where the two are
  the same.
...
This check reports whether the stamp moved, and never how far. [std-VERS] says which part a change moves, and a
reviewer judges whether a patch should have been a minor or a major.

The reach is a record. A framework page, a `README.md` and the files under `.schema/` sit outside an export, so
changing one of those asks for no move. A skill under `template/.plugin/` is the case this misses:
[std-VERS.what-a-move-of-each-stamp-means] asks every corpus whose bundle ships that skill to move, and the skill tree
sits in no corpus's folder.
