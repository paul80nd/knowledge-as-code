...
reader has installed nothing and has nobody here to ask. That is the whole difference from every other surface.
...
2. Check every factual claim against its source, including one you are only rewording. A rewrite drops a fact more
   easily than it drops a word, and the reader can check nothing.
3. Open on what the reader gets. A definition explains a term nobody has asked about yet.
4. Put each fact where its reader meets it. A flag met while running the tool belongs at `--help` and in the reference.
   A page read before installing states what a reader needs to decide whether to adopt `kac`.
...
8. Run `mkdocs build --strict` where you touched the site. A dead link fails it. `NavigationTests` catches a page the
   nav does not list.
9. Run [prc-pull-request].

## Verification

The build reports no dead link, and every box on both conformance checklists is ticked. A reader who has installed
...
* [std-A11Y] states what a page must give a reader who cannot see it. Step 6 is where you answer it.
