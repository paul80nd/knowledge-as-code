...
The push step asks the registry which versions it already holds, and stops where this one is among them. A registry
keeps a published version forever, so a correction is a new `content-version` and never a second push of this one.
...
* **One record covers four packages.** They differ in the content they carry and in nothing else: one repository, one
  owner, one workflow and one platform. [The type page](../services.md) carries the rule this record stands on.
...
* **`criticality` sits level with [svc-marketplace], though nothing here restores from the feed.**
  `examples/payments` and `examples/dog-fooding` name `../engineering/.dist/package` as their `source:`, so every build
  in this repository reads a folder. Somebody who cannot restore a package is outside this repository, so the impact is
  not internal only.
* **A pack is refused without a shortcode**, so a corpus joins the publishing matrix by declaring one.
