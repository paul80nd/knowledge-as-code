...
* **`monitoring-output` is `none`, and CI warns about it.** Nothing watches nuget.org, so a broken package reaches a
  person only when somebody installs it and says so. `critical-service-is-alerted` reports it on every run.
