...
| `<!-- … schema-<type> -->` block in `<type>.md` | the type's `fields`                 | The frontmatter reference table. Each row renders the field's `description`, falling back to `notes` where the schema declares none.                                                            |
