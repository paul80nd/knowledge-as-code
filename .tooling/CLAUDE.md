...
`Tree` is the same bargain for the corpus itself: a listing and a `Func` that reads one of its paths, which is what lets
`GeneratedFiles.Plan` and the link pass be asked about a corpus that was never written to disk.
...
`validate` ask what a file should carry without building any of it, and `Plan` renders them against what the corpus
holds now — so `index` and `index --check` read the same answer rather than each deciding what is stale.
