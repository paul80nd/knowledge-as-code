Feature: Cross-document graph checks
  kac reconciles ids across the whole wiki: uniqueness, reciprocal fields, and related/section
  mirroring. Driven in-process against the graph fixture.
  ...
  Scenario: The whole graph produces exactly these findings and nothing else
    ...
    And the findings are exactly:
      ...
      | adrs/0001-first.md | error    | 25   | link-resolves           | link target 'nonexistent-target.md' does not resolve.                                               |
      | adrs/0001-first.md | error    | 25   | undefined-label         | reference '[ADR-0099]' has no link definition.                                                      |
      | adrs/0001-first.md | warning  | 26   | bracket-literal         | '[an unlinked placeholder]' looks like a reference but has no definition (or use an inline link).   |
