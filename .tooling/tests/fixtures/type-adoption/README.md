...
`.mechanism.lock` adopts `runbooks` and `widgets`. The corpus holds neither, and holds `adrs`, which it has not
adopted.

* **`runbooks` is adopted and not stood up** — the state a sync exists to resolve, so it reads as work outstanding.
* **`widgets` is adopted and no schema covers it** — the lock names a type the corpus was never given.
...
The findings land against `.mechanism.lock` rather than against `.schema/`: what is wrong is the declaration, and the
