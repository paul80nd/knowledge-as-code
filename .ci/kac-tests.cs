...
//
// Each fixture under .ci/tests/fixtures/<scenario>/ is a deliberately-broken (or deliberately-clean)
...
//
//   dotnet run .ci/kac-tests.cs                 # run every scenario, fail on any mismatch
//   dotnet run .ci/kac-tests.cs -- clean        # run only scenarios whose name contains "clean"
//   dotnet run .ci/kac-tests.cs -- --update     # regenerate every golden from current kac output
//
...
void RunValidateScenario(string name, string scenario, string corpusDir)
{
    ...
    var actualJson = RunValidate(kac, schemaDir, corpusDir);
    ...
    {
        failures.Add(name);
        Console.WriteLine($"MISSING {name}  — no golden. Run: dotnet run .ci/kac-tests.cs -- --update {name}");
        return;
    }
...
void RunIndexScenario(string name, string scenario, bool mustBeStale)
{
    ...
    {
        failures.Add(name);
        Console.WriteLine($"FAIL   {name}  — committed generated files do not match the generator. Run: dotnet run .ci/kac-tests.cs -- --update {name}");
...
static string RunValidate(string kac, string schemaDir, string corpusDir)
{
