...
Serves every jacket image that appears in a result list, on an item page or on a branch terminal, resized and
re-encoded to the dimensions the request asks for — the `?width=320&quality=80` form seen throughout the estate.
...
**`repo` under-answers for this service, and the record says so rather than hiding it.** The field names
`infrastructure`, the repository that defines the edge, because that is the repository a change to *this service* is
made in and the answer has to be consistent across the estate. It does not answer "where do I change what this
serves" — that has two answers above and neither of them is `infrastructure`. One required string cannot carry both
questions, so the second is answered here in prose.
...
* [svc-thumbnailer] — the origin the edge forwards to. Configured as the CDN origin host, not as an application
  setting, so this dependency lives in the routing rather than in either service's configuration.
