...
4. **Load `technical-writing`, then `writing-a-record`.** Read the section for the tier the type carries, not the one
   you are used to.
5. **Copy the template and fill it.** Keep every required section. A section with nothing to say is a content gap to
   report, never a heading to delete.
6. **Write the frontmatter last**, once the record has settled. Field order is topological and `key-order` checks it.
7. **Run `kac validate`, then `kac generate`.** The record's H1 lands in a generated index, so a new record leaves the
