...
1. **Where a page already exists, read it cold before you open anything else.** Write down what a reader knows after
   the first paragraph, after the second, and after the first section. That is the diagnosis, and no measurement
   below will produce it: a page can sit inside every target and still fail to say what the thing is.
...
4. **Open on what the reader gets.** A definition answers a question nobody has asked yet, and a problem statement
...
6. **Measure the prose alone**, with code blocks and tables excluded, before and after. Under four contrasts per
   ...
   file you are editing may be the one the register was read from.
   **The measurement is a prompt to look, never the diagnosis.** A page inside both targets can still open on abstract
   nouns and leave a reader unable to say what it is.
7. **Pack the package**, where you touched `PACKAGE.md`. `dotnet pack tooling/kac/kac.csproj` proves it still renders
