...
8. **Copy the overlay files across.** `knowledge-as-code/**` and `glossary/knowledge-as-code.md` live in every tree,
   and `kac update --check --from ../../` inside each corpus under `examples/` holds them equal. `.schema/**` is
   authored once at the root, so it needs no copy. A root page and a `_template.md` are `seed`, so nothing catches
   drift and the copy is yours.
