...
The types declaring no block are `integrations` and `data`. Neither is a decision, and nothing here is deliberately
held back. No corpus in this repository has a record of either, so nothing here could prove the block was written
right.
