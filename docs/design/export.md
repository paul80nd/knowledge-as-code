...
The types declaring no block are `integrations`, `data` and `explanations`. None of those is a decision, and nothing
here is deliberately held back. No corpus in this repository has a record of one of them, so nothing here could prove
