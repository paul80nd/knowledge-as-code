...
this corpus cannot show. Three more keys travel beside them. `evidence` says where the proof lives. `last-verified`
says when a manual check last ran, so a reader can tell a check that happened from one that was only planned.
`review-by` says when somebody reads the record against the check it describes, which is a different question from how
