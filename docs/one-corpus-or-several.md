...
over every one of them on every commit.
...
* **`payments/`** is a domain corpus, and thin on purpose. It consumes `engineering/` and holds nothing that corpus
  already says.
* **`dog-fooding/`** is a domain corpus too, consuming `engineering/` as `payments/` does. Its subject is `kac`
  and the repository it sits in. Nothing else here holds this project to the framework it publishes.
