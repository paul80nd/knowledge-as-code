...
> **This type arrives with no records and three vocabularies to set.** `platform`, `component-type` and the facet list
> are each derived from the estate that adopts the type, and this file states none of them. The three sections below say
> how to derive your own. Set all three before you write your first service.
...
| `component-type` *  | stated by this corpus                    | The sort of component this is, which `platform` does not say.                                        |
| `repos` *           | list                                     | Repositories a change to this service is made in.                                                    |
| `platform` *        | stated by this corpus                    | What it is built on. Drives which standards apply.                                                   |
...
A template has no estate, so this one states no values. Yours is the first thing to write when you add a service.
...
A template has no estate, so this one states no values either.
...
| `dependency-criticality`      | warning | No record is graded above the criticality of anything it depends on.                                            |
