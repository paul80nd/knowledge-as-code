...
* **Move to a hosted knowledge platform (Confluence, SharePoint, Notion)** — better discovery, better editing, broader
  ...
  problem is the half we cannot currently solve at all, a platform that structurally forbids it cannot be the answer.
...
* **Adopt an off-the-shelf agent memory tool** — a ready-made vault with save/load/pin verbs. Rejected as a dependency:
  these are personal-scope tools with no multi-writer model, no review and no conflict handling. The underlying ideas —
  human-readable Markdown, explicit promotion of an insight into durable knowledge — are good, and we adopt them. The
  dependency is not worth taking for what amounts to a directory convention and three skills.
