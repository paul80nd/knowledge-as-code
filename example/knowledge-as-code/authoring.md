...
> What a record's tier asks of its prose.

[Style](style.md) is the floor under this page and binds every document in this repository. Read it
first. [Contributing](contributing.md) covers the mechanics and says which page owns what.
...
* **How a record is written follows its tier, not its type.** A runbook and a process are read under different
...
sentence meant. Length is not thoroughness. A record that says less, and says it once, is read.
...
**Precedence.** Where a rule here contradicts the schema, the schema is right and this page is wrong. It is executable
and this is not. Report the contradiction; do not resolve it by editing records.
...
Style applies throughout, and each tier adds to it. Your record's frontmatter carries its `tier`, so read the row that
...
* **A runbook opens with Symptoms**, because that is how a reader who does not yet know which record they need finds it.
  `symptoms-first` is an error, not a warning.
...
2119 keywords. Every normative type answers to the same sentence: state the thing a reader will act on, make it
checkable, and keep the argument out of the part that gets quoted.
...
* **One obligation per clause.** If a clause cannot be failed in exactly one way, split it — the `clause-order` and
  `clause-compound` checks warn when a row carries more than one.
...
* **Write the clause so it cannot be read two ways.** Then read it again, deliberately, looking for the second way.
...
* **Link rather than restate.** A capability and an explanation are hubs, and their length is judged against how much
  they link. The `hub-not-specification` and `links-rather-than-restates` checks warn at roughly forty words per
  outbound link. A hub that accumulates facts of its own has become a liability. That is what the ratio measures. The
  other descriptive types have no such rule and are held to it by judgement alone.
...
* **Length is earned per point, not per record.** Give each point the detail it needs and make it once.
...
* **Do not tidy a discovery into authority.** A discovery that reads like an FAQ will be trusted like one. Promotion is
  where certainty is added.
...
The test is repetition: if a paragraph would appear in more than one record, it is not record content. An explanation of
what `aligns-with` means, or of the difference between alignment and certification, is a corpus concern. That
explanation belongs [here](metadata.md), written once; repeated in every policy, it is twenty copies to keep in step and
