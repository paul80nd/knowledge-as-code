...
{
    // The day every case here is judged against, named rather than read from the clock, so a rule about a
    // date reads the same on any day the suite runs.
    private static readonly DateOnly RunDay = new(2026, 6, 15);
    ...
    private static Facts FactsFor(string frontmatter, string body, DateOnly? today = null)
    {
        var doc = Required.Parsed("adrs/0001-a-title.md",
            $"---\n{frontmatter}\n---\n\n# A title\n\n{body}", new Schema());
        return new Facts(doc, today ?? RunDay);
    }
    ...
    // comparison, which is the bargain the grammar already strikes between two date fields.
    [Fact]
    public void Today_is_the_run_day_as_an_iso_date()
        => Assert.True(Eval("today() == '2026-06-15'"));

    // The reading `expiry` is built on, both ways round and on the day itself, where a review is due
    // rather than late.
    [Theory]
