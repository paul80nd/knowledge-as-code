...
Scenario: An unused link definition and a verdict-less alternative both warn
  ...
  Then the findings for "adrs/0001-warnings.md" are exactly:
    ...
    | warning  | 30   | alternatives-verdict | Alternatives Considered bullet has no verdict: "A message queue  — we might explore this in a future revi…". |
...
Scenario: A list out of alphabetical order warns once, naming the entry that belongs earlier
  ...
  Then the findings for "adrs/0002-list-order.md" are exactly:
    | severity | line | check      | message                                                                              |
    | warning  |    9 | list-order | 'tags' is not in alphabetical order: 'access-control' should come before 'identity'. |
...
Scenario: A Y-statement past the ceiling its type declares warns, naming the count and the limit
  ...
  Then the findings for "adrs/0003-long-y-statement.md" are exactly:
    | severity | line | check       | message                                    |
    | warning  |   14 | y-statement | Y-statement is 95 words; keep it under 60. |
...
Scenario: A Y-statement short of its moves warns, naming each move that is absent
  ...
  Then the findings for "adrs/0004-incomplete-y-statement.md" are exactly:
    | severity | line | check       | message                                                                                                                       |
    | warning  |   14 | y-statement | Y-statement is missing "facing" and "rather than". The six moves are what make it a Y-statement rather than a summary of one. |
...
Scenario: The corpus as a whole warns seven times and errors not at all
  ...
  And the findings are exactly:
    ...
    | adrs/0001-warnings.md                | warning  | 30   | alternatives-verdict | Alternatives Considered bullet has no verdict: "A message queue  — we might explore this in a future revi…".                                                                                      |
    | adrs/0002-list-order.md              | warning  | 9    | list-order           | 'tags' is not in alphabetical order: 'access-control' should come before 'identity'.                                                                                                              |
    | adrs/0003-long-y-statement.md        | warning  | 14   | y-statement          | Y-statement is 95 words; keep it under 60.                                                                                                                                                        |
