...
Scenario: Every file carrying a generated block is checked for its markers, and for its links
  ...
  And the findings are exactly:
    ...
    | glossary/knowledge-as-code.md | 19   | link-resolves         | link target '/nope.md' does not resolve.                                                                                                                                                              |
    | glossary/knowledge-as-code.md | 19   | framework-names-types | '/adrs.md' links to the 'adrs' type from a document every corpus shares. Name the type instead: a corpus that has not adopted it reads a dead link, and one that has is no worse off.                 |
...
Scenario: A type's template is checked without being counted as a record
  ...
  And the findings are exactly:
    ...
    | adrs/_template.md |    5 | template-fields | 'decided-on' is read as a YAML mapping rather than a value. A placeholder that opens one has to be quoted: decided-on: "{{…}}". |
    | adrs/_template.md |   22 | link-resolves   | link target '0404-superseded-and-deleted.md' does not resolve.                                                                  |
