...
Scenario: Terms are addressed by the anchor their heading slugs to
  ...
  Then the findings for "glossary/dupe-two-terms-alike.md" are exactly:
    | line | check          | message                                                                                     |
    |   27 | part-id-unique | two terms here address as 'identity-line': a citation of it names both and reaches neither. |
  And the findings for "glossary/tref-unknown-term.md" are exactly:
    | line | check    | message                                                                                                                   |
    |   25 | part-ref | 'gls-dupe-two-terms-alike.no-such-term' cites a term 'no-such-term' that glossary/dupe-two-terms-alike.md does not carry. |
...
Scenario: A parts section holding no headings is told what belongs there
  ...
  Then the findings for "glossary/none-terms-section-empty.md" are exactly:
    | line | check     | message                                                                 |
    |   21 | part-none | the '## Terms' section holds no terms. Write each one as an H3 heading. |
...
Scenario: The whole corpus produces exactly these findings and nothing else
  ...
  And the findings are exactly:
    ...
    | glossary/holl-empty-term.md           | error    |   28 | part-empty       | term 'Hollow' has nothing under it. Write it or delete the heading.                                                            |
    | glossary/holl-empty-term.md           | error    |   32 | part-empty       | term 'Placeholder' has nothing under it. Write it or delete the heading.                                                       |
    | glossary/none-terms-section-empty.md  | error    |   21 | part-none        | the '## Terms' section holds no terms. Write each one as an H3 heading.                                                        |
    | glossary/tref-unknown-term.md         | error    |   25 | part-ref         | 'gls-dupe-two-terms-alike.no-such-term' cites a term 'no-such-term' that glossary/dupe-two-terms-alike.md does not carry.      |
