...
Any. The finding is about the string, and nothing in the environment bears on it.
...
Write each `at` as `YYYY-MM-DDThh:mm:ssZ`, in UTC. The entries read in order, so `list-order` stays quiet and each
finding is about the moment beneath it.
