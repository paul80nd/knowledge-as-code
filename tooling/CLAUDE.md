...
| **Cross-document** | `alert-after-needs-an-alert` (**written**) beside `rules-have-controls`, `redefinitions-are-reciprocal`, `undefined-terms`, `unused-terms` | Each fits `ICorpusRule`. `redefinitions-are-reciprocal` is the hardest: it holds an entry inside one document against an entry inside another. |
| **Graph**          | `no-dependency-cycles` and `dependency-criticality`, both **written**                                                                                       | Each asks about an edge, and neither end answers it. A loop lives in the set of edges, and the grade a record is held against sits on the document at the far end.                                                     |
| **Per-part**       | `alternatives-have-verdicts`, `terms-are-alphabetical`, `escalation-required`, `definitions-do-not-repeat-the-term` and `entries-state-no-requirement` (**written**) beside `carried-in-full-by-digest`     | Each judges the parts of one document: bullets under a heading, entries in a glossary, branches of a diagnosis tree. Its message has to name the part that failed.                                                     |
