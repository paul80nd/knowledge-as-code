...
| **Cross-document** | `alert-after-needs-an-alert` (**written**) beside `not-load-bearing`, `constraint-consistency`, `rules-have-controls`, `redefinitions-are-reciprocal`, `undefined-terms`, `unused-terms` | Each fits `ICorpusRule`. `redefinitions-are-reciprocal` is the hardest: it holds an entry inside one document against an entry inside another. |
...
| **Per-part**       | `alternatives-have-verdicts` and `terms-are-alphabetical` (**written**) beside `carried-in-full-by-digest`, `escalation-required`     | Each judges the parts of one document: bullets under a heading, entries in a glossary, branches of a diagnosis tree. Its message has to name the part that failed.                                                     |
