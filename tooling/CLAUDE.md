...
| **Per-part**       | `alternatives-have-verdicts`, `terms-are-alphabetical` and `escalation-required` (**written**) beside `carried-in-full-by-digest`     | Each judges the parts of one document: bullets under a heading, entries in a glossary, branches of a diagnosis tree. Its message has to name the part that failed.                                                     |
